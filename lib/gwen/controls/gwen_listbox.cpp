...
{
	m_SelectedRows.erase( stl::find( m_SelectedRows.begin(), m_SelectedRows.end(), row ) );
	m_Table->Remove( row );
}
...
{
	stl::list<Layout::TableRow*>::iterator it = m_SelectedRows.begin();
